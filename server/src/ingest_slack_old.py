#!/usr/bin/env python3
"""
Slack messages ingestion script for Graphiti MCP.
...
def get_user_info(token: str, user_id: str, user_cache: dict) -> str:
...
async def ingest_slack_messages(
    ...
    # Fetch messages
    messages = fetch_slack_messages(token, channel_id, workspace_id, oldest=oldest)
    ...
    for thread_ts, thread_msgs in tqdm(threads.items(), desc="Ingesting threads"):
        try:
            ...
            for msg in thread_msgs:
                ...
                user_name = get_user_info(token, user_id, user_cache)
                text = msg.get("text", "")
