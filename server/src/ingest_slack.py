#!/usr/bin/env python3
"""
Slack messages ingestion script for Graphiti MCP with English translation.
"""
...
async def ingest_slack_messages(
    ...
    """Ingest Slack messages into Graphiti with English translation."""
    now = datetime.now()
    ...
    if save_to_disk:
        ...
        filename = f"messages_{channel_id}_{timestamp}_{'en' if translate else 'original'}.json"
        ...
        try:
            ...
            for msg in thread_msgs:
                ...
                if translate and text:
                    text = translate_with_limit(text, max_chars=1000)
...
async def main():
    ...
    parser.add_argument("--cookie", type=str, default=None, help="Cookie string for authentication")
    parser.add_argument("--days", type=int, default=1, help="Number of days to fetch")
    parser.add_argument("--clear-existing", action="store_true", help="Clear existing graph data")
    parser.add_argument("--no-save", action="store_true", help="Do not save messages to disk")
    parser.add_argument("--no-translate", action="store_true", help="Do not translate to English")
